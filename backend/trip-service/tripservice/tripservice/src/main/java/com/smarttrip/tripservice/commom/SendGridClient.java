...
public class SendGridClient {
    ...
    private final String apiKey = System.getenv("SENDGRID_API_KEY");
    private final String fromEmail = System.getenv("SENDGRID_FROM");

    // 📌 send basic email
    public boolean sendEmail(String to, String subject, String html) {
        try {
            Email from = new Email(fromEmail, "SmartTrip");
            Email toEmail = new Email(to);
    ...
    public boolean sendEmailWithAttachment(String to, String subject, String html, byte[] pdfBytes) {
        try {
            Email from = new Email(fromEmail, "SmartTrip");
            Email toEmail = new Email(to);
            ...
            if (pdfBytes != null && pdfBytes.length > 0) {
                Attachments attachment = new Attachments();
                attachment.setContent(java.util.Base64.getEncoder().encodeToString(pdfBytes));
                attachment.setType("application/pdf");
                attachment.setFilename("SmartTrip-Invoice.pdf");
                attachment.setDisposition("attachment");
            ...
            System.out.println("❌ SendGrid Attachment Error: " + ex.getMessage());
            return false;
        }
    }
}